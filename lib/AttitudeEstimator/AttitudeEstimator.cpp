...
void AttitudeEstimator::update(double dt, double gyroX, double gyroY, double gyroZ, double accX, double accY, double accZ, double magX, double magY, double magZ)
{
  ...
  if (qscale < QHAT_NORM_TOL_SQ)
  {
    reset(true);
    return;
  } // The quaternion is so far away from being normalised (almost zero norm when it should be 1) that something must be dreadfully wrong... (avoid potential division by zero below)
  qscale = 1.0 / sqrt(qscale);
  m_Qhat[0] *= qscale;
  m_Qhat[1] *= qscale;
  m_Qhat[2] *= qscale;
