...
#ifndef _ATTITUDE_ESTIMATOR_H
#define _ATTITUDE_ESTIMATOR_H
...
namespace stateestimation
{
  ...
  public:
    ...
    void getAttitude(double q[]) const
    {
      for (int i = 0; i < 4; i++)
        q[i] = m_Qhat[i];
    }                                                                           //!< @brief Returns the current attitude estimate in the quaternion form (w,x,y,z) (Note: `q[]` must have room for 4 elements).
    ...
    double eulerYaw()
    {
      ...
      return m_Ehat[0];
    } //!< @brief Returns the ZYX Euler yaw of the current attitude estimate (1st of the three ZYX Euler angles).
    double eulerPitch()
    {
      if (!m_eulerValid)
      {
      ...
      return m_Ehat[1];
    } //!< @brief Returns the ZYX Euler pitch of the current attitude estimate (2nd of the three ZYX Euler angles).
    double eulerRoll()
    {
      if (!m_eulerValid)
      {
      ...
      return m_Ehat[2];
    } //!< @brief Returns the ZYX Euler roll of the current attitude estimate (3rd of the three ZYX Euler angles).
    double fusedYaw()
    {
      if (!m_fusedValid)
      {
      ...
      return m_Fhat[0];
    } //!< @brief Returns the fused yaw of the current attitude estimate (1st of the fused angles).
    double fusedPitch()
    {
      if (!m_fusedValid)
      {
      ...
      return m_Fhat[1];
    } //!< @brief Returns the fused pitch of the current attitude estimate (2nd of the fused angles).
    double fusedRoll()
    {
      if (!m_fusedValid)
      {
      ...
      return m_Fhat[2];
    } //!< @brief Returns the fused roll of the current attitude estimate (3rd of the fused angles).
    bool fusedHemi()
    {
      if (!m_fusedValid)
      {
      ...
      return m_FhatHemi;
    } //!< @brief Returns the hemisphere of the current attitude estimate (boolean 4th parameter of the fused angles representation, where `true` implies `1` and `false` implies `-1`).
    ...
    void getGyroBias(double b[]) const
    {
      for (int i = 0; i < 3; i++)
        b[i] = m_bhat[i];
    } //!< @brief Returns the current estimated gyro bias (Note: `b[]` must have room for 3 elements).
    void setGyroBias(const double b[])
    {
      for (int i = 0; i < 3; i++)
        m_bhat[i] = b[i];
    } //!< @brief Resets the current estimated gyro bias to a particular vector value (Note: `b[]` must have 3 elements).
    void setGyroBias(double bx, double by, double bz)
    {
      m_bhat[0] = bx;
      m_bhat[1] = by;
      m_bhat[2] = bz;
    } //!< @brief Resets the current estimated gyro bias to a particular vector value.
    ...
    void getMagCalib(double mt[]) const
    {
      for (int i = 0; i < 3; i++)
        mt[i] = m_magCalib[i];
    } //!< @brief Returns the current magnetometer calibration vector. This should be the value of `(magX,magY,magZ)` that corresponds to a true orientation of identity (Note: `mt[]` must have room for 3 elements).
    void setMagCalib(const double mt[])
    {
      for (int i = 0; i < 3; i++)
        m_magCalib[i] = mt[i];
    } //!< @brief Sets the magnetometer calibration vector to use in the update functions. This should be the value of `(magX,magY,magZ)` that corresponds to a true orientation of identity (Note: `mt[]` must have 3 elements).
    void setMagCalib(double mtx, double mty, double mtz = 0.0)
    {
      m_magCalib[0] = mtx;
      m_magCalib[1] = mty;
      m_magCalib[2] = mtz;
    } //!< @brief Sets the magnetometer calibration vector to use in the update functions. This should be the value of `(magX,magY,magZ)` that corresponds to a true orientation of identity.
    ...
    void setQLTime(double QLTime)
    {
      if (QLTime > 0.0)
        m_QLTime = QLTime;
    } //!< @brief Sets the quick learning time to use. See `getQLTime()` for more details (Note: Non-positive values of `QLTime` are ignored by this function).
